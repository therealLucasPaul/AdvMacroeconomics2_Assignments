...
var A V PEreal PI R Y C N MCreal Wreal EN Rreal;

varexo ev ea een;

parameters alfa phi betta sigma varphi mu xsi theta alpha lambda kappa phi_p phi_x rho_a rho_v sigma_a sigma_v rho_en
...
model;
    ...
    // labor supply: 
    exp(N)^varphi * exp(C)^sigma = exp(Wreal);

    // Marginal costs:
    exp(MCreal) = 1/exp(A)*(1/(1-alfa))^(1-alfa)*(1/alfa)^(alfa)*exp(Wreal)^(1-alfa)*exp(PEreal)^(alfa);

    // optimal pricing: 
    theta*exp(MCreal) - (theta-1) = phi*(exp(PI)-1)*exp(PI) - betta*phi*(exp(C(+1))/exp(C))^(-sigma)*(exp(PI(+1))-1)*exp(PI(+1))*(exp(Y(+1))/exp(Y));

    // Euler equation
    1 = betta* (exp(C(+1))/exp(C))^(-sigma) * (exp(R)/exp(PI(+1)));

    // resource constraint FERTIG
    exp(C) = exp(Y) - exp(PEreal)*exp(EN)- phi/2 * (exp(PI)-1)^2 * exp(Y); 

    //production function
    exp(Y) = exp(A) * exp(N)^(alfa) * exp(EN)^(1-alfa);
    
    // Labour Energy Ration FERTIG
    exp(N)/exp(EN) = (1-alfa)/(alfa) * (PEreal/Wreal);
    ...
    // Exogenous Process for Energy Price Shock
    PEreal = rho_en * PEreal(-1) + een;
