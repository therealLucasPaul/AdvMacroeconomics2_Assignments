...
//
// Adaption for the Assignment made by Sophia Oberbrinkmann und Lucas Unterweger


var PI R V A Y C N MC W Rreal EN PE;
...
model;
    ...
    // New Marginal Costs Equation:
    exp(MC) = (exp(PE)/alpha)^alpha * (exp(W)/(1-alpha))^(1-alpha) *(1/exp(A));
    ...
    exp(C) + phi/2 * (exp(PI)-1)^2 * exp(Y) + exp(PE) * exp(EN) = exp(Y); 
...
stoch_simul(irf=20, order=1) PI R V A Y C N MC W EN PE;      
